/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/CoreIndoor.framework/CoreIndoor
 */

@interface MultiphoneDbmParameters : PBCodable <NSCopying> {
    struct { 
        unsigned int searchWidth : 1; 
        unsigned int variancePerSecond : 1; 
    } _has;
    float _searchWidth;
    float _variancePerSecond;
}

@property(assign,readwrite) bool hasSearchWidth;
@property(assign,readwrite) bool hasVariancePerSecond;
@property(assign,readwrite) float searchWidth;
@property(assign,readwrite) float variancePerSecond;

- (void)copyTo:(id)arg1;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)description;
- (id)dictionaryRepresentation;
- (bool)hasSearchWidth;
- (bool)hasVariancePerSecond;
- (unsigned long long)hash;
- (bool)isEqual:(id)arg1;
- (void)mergeFrom:(id)arg1;
- (bool)readFrom:(id)arg1;
- (float)searchWidth;
- (void)setHasSearchWidth:(bool)arg1;
- (void)setHasVariancePerSecond:(bool)arg1;
- (void)setSearchWidth:(float)arg1;
- (void)setVariancePerSecond:(float)arg1;
- (float)variancePerSecond;
- (void)writeTo:(id)arg1;

@end
